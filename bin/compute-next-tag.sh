...
#
# Tags look like `v<Gitea version>-<release>`:
#
# - if defaults/main.yml points at a Gitea version that has never been
#   released, the release counter restarts at 0 (`v1.27.3-0`)
# - otherwise the counter is incremented (`v1.27.3-1`), but only if something
...
# Anchored at the start of the line, so that variables deriving from this one
# (`gitea_container_image_tag: "{{ gitea_version }}-rootless"`) cannot be
# mistaken for the version itself.
version="$(sed -nE 's|^gitea_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Gitea version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `1.27.2`), but the tags
# do (`v1.27.2-0`). Stripping any `v` before prepending one keeps this correct
